...
public class BillionairesDaoImpl implements BillionairesDao {
	...
	public boolean updateBillionaire(Billionaires billionaires) {
		return jdbcTemplate.update("update people set first_name = ?, last_name = ?, age  = ? where id = ?", billionaires.getFirstName(), billionaires.getLastName(), 
				billionaires.getCareer(),billionaires.getId()) > 0;	}
